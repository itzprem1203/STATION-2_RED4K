...
def measurement_count(request):
    if request.method == 'POST':# Retrieve input parameters
        ...
        # Query MeasurementData to fetch date and part_status
        filtered_data = (
            MeasurementData.objects
            .filter(part_model=input_partModel,date__date=formatted_date, shift=input_shift)  # Fetch date and part_status
            .values('date', 'part_status')
            .annotate(count=Count('part_status'))  # Count occurrences of part_status
            .order_by('date')  # Sort by date
        )
        print('your data for thsi filtered data is this:',filtered_data)
        ...
        for entry in filtered_data:
            status = entry['part_status'].lower()  # Convert status to lowercase for comparison
